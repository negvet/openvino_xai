...
class ViTReciproCAM(FeatureMapPerturbationBase):
    ...
    def _get_saliency_map(self, model_clone: ov.Model) -> ov.Node:
        ...
        norm_node_ori = post_target_node_ori[0]
        while norm_node_ori.get_type_name() != "Add":
            if len(norm_node_ori.outputs()) > 1:
                raise ValueError
            inputs = norm_node_ori.output(0).get_target_inputs()
            ...
            if len(inputs) == 1:
                ...
                for input in inputs:
                    ...
                    if input_node.get_type_name() == "MVN":
                        norm_node_ori = input_node
                        break
...
class ViTReciproCAMSwin(ViTReciproCAM):
    """
    Implements ViTRecipro-CAM for transformer models.
    ...
    def _get_saliency_map(self, model_clone: ov.Model) -> ov.Node:
        ...
        mosaic_feature_map_norm = self._get_mosaic_feature_map(norm_node_ori, dim, h, w)
        for node in post_target_node_clone_norm:
            node.input(0).replace_source_output(mosaic_feature_map_norm.output(0))

        # Mosaic feature map after the Add node
        mosaic_feature_map = self._get_mosaic_feature_map(target_node_ori, dim, h, w)


        reshape_node = post_target_node_clone[0]
        inputs = reshape_node.output(0).get_target_inputs()
        for input in inputs:
            input_node = input.get_node()
            if input_node.get_type_name() == "Add":
                add_node_clone = input_node
                break
        # add_node_clone = self._get_add_node_from_two_nodes(post_target_node_clone)
        add_node_clone.input(0).replace_source_output(mosaic_feature_map.output(0))
    ...
    def _get_mosaic_feature_map(self, target_node_ori: ov.Node, dim: int, h: int, w: int):
        if self._use_gaussian:
            # if self._cls_token:
            #     cls_token = opset.slice(
            #         target_node_ori, np.array([0]), np.array([num_aux_tokens]), np.array([1]), np.array([1])
            ...
            # )
            squeezed = False
            if len(target_node_ori.get_output_partial_shape(0)) == 3:
                target_node_ori = opset.reshape(target_node_ori, (1, h, w, dim), False)
                squeezed = True
            feature_map_spacial_repeated = opset.tile(target_node_ori.output(0), (h * w, 1, 1, 1))
            ...
            mosaic_feature_map = opset.multiply(feature_map_spacial_repeated, mosaic_feature_map_mask)
            if squeezed:
                mosaic_feature_map = opset.reshape(mosaic_feature_map, (h * w, h * w, dim), False)
            # mosaic_feature_map = opset.concat([cls_token, mosaic_feature_map], 1)
        # else:
        #     mosaic_feature_map_mask_wo_cls_token = np.zeros((h * w, h * w), dtype=np.float32)
        #     for i in range(h * w):
        #         mosaic_feature_map_mask_wo_cls_token[i, i] = 1
